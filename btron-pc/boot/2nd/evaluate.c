...
/* --- command adapters for functions with different signatures --- */
static int ide_read_dump_cmd (char **args)
{
  /* `ide_read_dump` expects `char **` and returns void */
  ide_read_dump (args);
  return E_OK;
}

static int k101us_cmd (char **args)
{
  k101us(args);
  return E_OK;
}

static int k106jp_cmd (char **args)
{
  k106jp(args);
  return E_OK;
}
