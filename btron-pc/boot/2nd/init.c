...
int init_main(void)
{
    ...
    /* SFS を登録してルートマウントを試みる（デバイスID 0 を想定） */
    if (vfs_register_fs(FS_TYPE_SFS, &sfs_ops) != 0) {
        console_printf("init: failed to register SFS\n");
    }

    if (vfs_mount("/", 0, FS_TYPE_SFS) != 0) {
        console_printf("init: failed to mount root filesystem on device 0\n");
    } else {
        console_printf("init: root filesystem mounted\n");
    }

    /* コンソールが標準入出力として使える前提でシェルを起動 */
    shell_main();

    /* shell が戻ったらシャットダウン代わりにループ */
    console_printf("init: shell exited\n");
    while (1) {
        asm("hlt");
    }
