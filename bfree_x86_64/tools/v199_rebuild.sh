...
cd "$ROOT"
sed -i 's/\r$//' tools/patch_qv4internalclass_init_copy_guest.sh
bash tools/patch_qv4internalclass_init_copy_guest.sh
...
BFREE_SMOKE_REBUILD=0 BFREE_FAST_TIMEOUT=300 bash tools/guest_desktop_smoke.sh 2>&1 | tee /tmp/v199-smoke.log | tail -95
