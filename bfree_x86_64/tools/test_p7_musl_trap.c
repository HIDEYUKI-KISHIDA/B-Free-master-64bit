/*
 * P7_MUSL_TRAP — musl static via trap-init child exec shim.
 */
...
int main(void)
{
	...
	bfree_trap_init();
	bfree_fs_init(&fs);
	CHECK(copy_file_to_fs(&fs, MUSL_STATIC_ELF_PATH, "/tmp/musl_trap") == 0,
	      "stage musl");
	CHECK(bfree_as_init(&as, 65536) == 0, "as init");
	CHECK(bfree_elf_load_path(&fs, "/tmp/musl_trap", &as, &img) == 0, "load");
	CHECK(bfree_elf_trap_exec(&as, img.entry, img.load_size, &status) == 0,
	      "trap exec");
	CHECK(status == 0, "musl exit 0");

	printf("P7_MUSL_TRAP: PASS\n");
	bfree_as_free(&as);
	return 0;
}
