...
BD_QML="/root/out/bfree-qt6-guest-static/build-qtdeclarative"
PREFIX="/root/out/bfree-qt6-guest-static"

cd "$ROOT"
for f in tools/patch_qqmlimport_guest.sh tools/patch_qqmlimport_addpath_guest.sh tools/patch_qbindingstorage_guest.sh; do
  sed -i 's/\r$//' "$f"
done
...
rm -f "$BD_CORE/src/corelib/CMakeFiles/Core.dir/kernel/qproperty.cpp.o"
cmake --build "$BD_CORE" --target Core -j4
cp -a "$BD_CORE/lib/libQt6Core.a" "$PREFIX/lib/"

rm -f "$BD_QML/src/qml/CMakeFiles/Qml.dir/qml/qqmlimport.cpp.o"
cmake --build "$BD_QML" --target Qml -j4
cp -a "$BD_QML/lib/libQt6Qml.a" "$PREFIX/lib/"
