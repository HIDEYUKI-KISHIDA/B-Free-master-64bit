...
cd "$ROOT"
for f in tools/patch_qv4internalclass_namemap_add_guest.sh \
    ...
    tools/patch_qv4addmember_heartbeat_guest.sh; do
  sed -i 's/\r$//' "$f"
  bash "$f"
done
...
BFREE_SMOKE_REBUILD=0 BFREE_FAST_TIMEOUT=300 bash tools/guest_desktop_smoke.sh 2>&1 | tee /tmp/v193-smoke.log | tail -70
