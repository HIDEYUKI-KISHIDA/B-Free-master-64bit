...
cd "$ROOT"
sed -i 's/\r$//' tools/fix_v254_resolve_callback_guest.py tools/v254_rebuild.sh
python3 tools/fix_v254_resolve_callback_guest.py
...
sed -i 's/mmap96-v[0-9]*/mmap96-v254/' userland/desktop_qt/guest_main.cpp
rm -f userland/desktop_qt/desktop.elf userland/desktop_qt/guest_link_compat.o
...
nm -C userland/desktop_qt/desktop.elf | grep 'bfree_guest_lookup_qml_type_id' | head -1
echo "[v254] done"
