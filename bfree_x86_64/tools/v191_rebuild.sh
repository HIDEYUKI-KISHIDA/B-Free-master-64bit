...
cd "$ROOT"
for f in tools/patch_qv4propertyhash_guest.sh tools/patch_qv4propertyhash_entries_guest.sh \
    tools/patch_qv4internalclass_init_other_guest.sh tools/patch_qv4internalclass_grow_guest.sh \
    tools/patch_qv4internalclass_namemap_add_guest.sh tools/patch_qv4internalclass_ptr_set_guest.sh \
    tools/patch_qv4namemap_heartbeat_guest.sh tools/patch_qv4namemap_sanitize_guest.sh \
    tools/patch_qv4addmember_heartbeat_guest.sh \
    tools/patch_qv4engine_arrayproto_guest.sh \
    tools/patch_qv4mm_allocate_guest.sh tools/patch_qv4mm_allocmanaged_ic_null_guest.sh \
    tools/patch_qv4memberdata_allocate_guest.sh tools/patch_qv4writebarrier_markcustom_guest.sh; do
  sed -i 's/\r$//' "$f"
  bash "$f"
done
rm -f "$BD/src/qml/CMakeFiles/Qml.dir/jsruntime/qv4internalclass.cpp.o" \
      "$BD/src/qml/CMakeFiles/Qml.dir/jsruntime/qv4memberdata.cpp.o" \
...
BFREE_SMOKE_REBUILD=0 BFREE_FAST_TIMEOUT=300 bash tools/guest_desktop_smoke.sh 2>&1 | tee /tmp/v191-smoke.log | tail -65
