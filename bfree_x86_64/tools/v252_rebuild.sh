...
cd "$ROOT"
sed -i 's/\r$//' tools/fix_v252_resolve_window_metaobject.py tools/v252_rebuild.sh
python3 tools/fix_v252_resolve_window_metaobject.py
...
echo "[v252] grub-mkrescue background pid=$!"
strings userland/desktop_qt/desktop.elf | grep 'mmap96-v' | head -1
echo "[v252] desktop.elf done"
