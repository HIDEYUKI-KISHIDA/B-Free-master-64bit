...
cd "$ROOT"
for f in tools/patch_qv4object_guest.sh tools/fix_asproto_guest.sh; do
    sed -i 's/\r$//' "$f"
done
...
BFREE_SMOKE_REBUILD=0 BFREE_SMOKE_TIMEOUT=600 bash tools/guest_desktop_smoke.sh 2>&1 | tee /tmp/v170-smoke.log
