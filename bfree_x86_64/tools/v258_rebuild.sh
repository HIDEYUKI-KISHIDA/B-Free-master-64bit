...
cd "$ROOT"
sed -i 's/\r$//' tools/fix_v258_*.py tools/v258_rebuild.sh
python3 tools/fix_v258_qqmlengine_register_qml.py
python3 tools/fix_v258_resolve_qtobject_metaobject.py || true
...
sed -i 's/mmap96-v[0-9]*/mmap96-v258/' userland/desktop_qt/guest_main.cpp
...
objdump -d userland/desktop_qt/desktop.elf | grep -A3 'qml_register_types_QML' | head -6
echo "[v258] done"
