...
cd "$ROOT"
sed -i 's/\r$//' tools/patch_qv4namemap_sanitize_guest.sh
bash tools/patch_qv4namemap_sanitize_guest.sh
...
BFREE_SMOKE_REBUILD=0 BFREE_FAST_TIMEOUT=300 bash tools/guest_desktop_smoke.sh 2>&1 | tee /tmp/v192-smoke.log | tail -65
