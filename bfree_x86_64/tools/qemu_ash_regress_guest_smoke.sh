#!/usr/bin/env bash
# P16_ASH_REGRESS_GUEST_QEMU — BusyBox ash regress on Linux stack+auxv boot.
set -euo pipefail
...
echo "== qemu_ash_regress_guest_smoke: build kernel (ash regress Linux-stack boot) =="
...
if [[ "${out}" == *"ASH_LINUX_STACK_BOOT"* &&
      "${out}" == *"ASH_PIPE_GUEST_OK"* &&
