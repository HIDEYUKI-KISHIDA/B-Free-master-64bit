...
echo "  strings kernel/kernel.elf | grep -E 'build=d2c-vfork-3|\\[VFORK\\] immute|\\[D3\\]'"
echo "  make -C userland/compositor_stub clean BFREE_D3=1 all"
echo "  BFREE_D2B_QML=0 bash tools/build_qt_wl_hello.sh   # optional; copy qt_wl_hello.elf if link fails"
...
echo "  bash tools/check_desktop_holder_embedded.sh   # must show sha256 OK"
