...
cmake --build . --target Core -j4 2>&1 | tail -4
...
BFREE_SMOKE_REBUILD=0 BFREE_SMOKE_TIMEOUT=480 bash tools/guest_desktop_smoke.sh > /tmp/v153-smoke.log 2>&1 || true
grep -E 'build=|QML ready|QQmlEngine ok|bridge|EXCEPTION|smoke\]|TIMEOUT|futex|nanosleep' /tmp/v153-smoke.log || true
f=$(ls -t /tmp/bfree-guest-smoke.* 2>/dev/null | head -1)
echo "LOG=$f"
grep -E 'build=|QML ready|QQmlEngine ok|bridge|qv4|EXCEPTION|event loop|memcpy|futex|nanosleep' "$f" | tail -80
