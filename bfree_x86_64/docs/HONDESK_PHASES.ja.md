...
| **4** | 日次から bfree QPA を外す。`DesktopShell.qml` を Wayland クライアントとして載せる | **D1 済 / D2 第一段済 / D2c 1024×768 bits / D2b 停止** | D2c はクライアントが画面を覆う。D2b は `QQmlEngine` ctor が戻らない。`BFREE_D2B_QML=0`。`beginCreate` しない。日次 `bfree.iso` は上書きしていない |
