...
| **4** | 日次から bfree QPA を外す。`DesktopShell.qml` を Wayland クライアントとして載せる | **D1 済 / D2 第一段済 / D2b 着手** | D2 bits + vfork parent。D2b は `QQmlEngine` のみ。`beginCreate` しない。日次 `bfree.iso` は上書きしていない。`execve("desktop.elf")` を g1-desk でやらない |
