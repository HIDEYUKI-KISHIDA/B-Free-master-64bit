...
| **3** | カーネル + **Qt Wayland**（bfree QPA を選ばない） | **いまここ / 止まっている** | 53MB `qt_wl_hello.elf` を `p8test.elf` に載せる。証明は金 `0xD4A017` / 紺 `0x1E3A8A` / シアン `0x06B6D4`。シリアルは `plugin registered` → `ctor ok` → `QPA wayland create` → `[wl] vfork parent`。古い ISO は `ctor mmap fail ret=0`（msync 26）。APP は syscall 9 + 32MiB ctor。灰色 wallpaper |
