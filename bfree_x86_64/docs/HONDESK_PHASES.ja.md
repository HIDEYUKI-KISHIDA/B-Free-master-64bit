...
| **4** | 日次から bfree QPA を外す。`DesktopShell.qml` を Wayland クライアントとして載せる | **D1 済 / D2 着手** | stub hello は `wayland`。D2 は QML を運んで GuestMvpShell bits（金窓ではない）。`beginCreate` しない。日次 `bfree.iso` は上書きしていない。`execve("desktop.elf")` を g1-desk でやらない |
