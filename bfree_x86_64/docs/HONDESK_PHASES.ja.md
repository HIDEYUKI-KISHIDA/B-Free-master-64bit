...
| **3** | カーネル + **Qt Wayland**（bfree QPA を選ばない） | **いまここ / ctor ok** | 53MB `qt_wl_hello.elf` を `p8test.elf` に載せる。証明は金 `0xD4A017` / 紺 `0x1E3A8A` / シアン `0x06B6D4`。`[qt] QGuiApplication ctor ok` まで緑。今は ctor のあと（window / paint / `processEvents`）で止まる |
