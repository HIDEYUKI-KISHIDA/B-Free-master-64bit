...
| **4** | 日次から bfree QPA を外す。`DesktopShell.qml` を Wayland クライアントとして載せる | **D1 いまここ** | 日次 `bfree.iso` は上書きしない。stub hello が `-platform wayland`。金窓は残す。D2 は QML。`execve("desktop.elf")` を g1-desk でやらない |
