...
| **3** | カーネル + **Qt Wayland**（bfree QPA を選ばない） | **済（W8）** | 金バー / 紺本体 / シアン印。`[wl] vfork parent`。C の緑ではない |
| **4** | 日次から bfree QPA を外す。`DesktopShell.qml` を Wayland クライアントとして載せる | **D1 済 / D2 第一段済** | stub hello は `wayland`。GuestMvpShell bits + `[wl] vfork parent`。`beginCreate` しない。日次 `bfree.iso` は上書きしていない。製品 QML IR はまだ。`execve("desktop.elf")` を g1-desk でやらない |
