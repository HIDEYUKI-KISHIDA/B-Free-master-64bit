...
| **4** | 日次から bfree QPA を外す。`DesktopShell.qml` を Wayland クライアントとして載せる | **D1 済 / D2 未** | stub hello は `wayland`。日次 `bfree.iso` は上書きしていない。D2 は QML。`execve("desktop.elf")` を g1-desk でやらない |
