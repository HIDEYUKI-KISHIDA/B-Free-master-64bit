...
| **本デスク** | 製品の机。compositor が画素を持ち、`DesktopShell.qml` が Qt Wayland クライアント | D1 済。D2 着手（QML を運ぶ。beginCreate しない）。D3 はまだ |
| **GPU** | 絵を GPU で出す | まだ。今は全部ソフトウェア FB |

**D1 済。D2 着手。** 日次 `bfree.iso` は上書きしない。GPU も触らない。`beginCreate` はやり直さない。
...
日次 N2 の机をこれに差し替える。W8 金窓・D1 wayland 済。**D2 着手（beginCreate しない）。** 日次 ISO は触らない。
