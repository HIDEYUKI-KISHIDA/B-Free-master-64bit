...
ゲート: `test_p19_linux_process_abi`（stack）/ `test_p20_user_va_brk_mmap`（L2）/ `test_p21_multi_mb_execve`（L3）/ `test_p22_proc_self_exe`（L4）/ `test_p23_procfs_basic` + `test_p24_procfs_runtime`（L5/L6）/ `test_p25_signal_abi_honesty`（L7）/ `test_p26_rt_sigreturn`（L8）/ `test_p27_signal_deliver`（L9）/ `test_p28_signal_entry`（L10）/ `test_p29_abi_thicken` / `test_p30_linux_compat`（互換1–5）+ QEMU ash（`ASH_LINUX_STACK_BOOT`）
