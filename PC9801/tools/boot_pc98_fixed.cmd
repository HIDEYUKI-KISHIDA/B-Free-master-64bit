...
if not exist "%BOOTIMG%" set "BOOTSIZE=0" & if exist "%BOOTIMG%" for %%F in ("%BOOTIMG%") do set "BOOTSIZE=%%~zF"
if not exist "%ITRONIMG%" set "ITRONSIZE=0" & if exist "%ITRONIMG%" for %%F in ("%ITRONIMG%") do set "ITRONSIZE=%%~zF"
